...
5,函数,2,3`;
        ...
        function buildBindSummaryToast(results) {
            ...
            const successCount = safeResults.filter(result => result && result.success).length;
            const failedResults = safeResults.filter(result => !result || !result.success);
            const failedCount = failedResults.length;
            if (failedCount === 0) return `成功 ${successCount}，失败 0`;

            const reasonCounts = new Map();
            failedResults.forEach(result => {
                const text = getBindFailureReasonText(result && result.reason);
        ...
        function createMapping() {
            ...
            const result = attemptBind(state.selectedA, state.selectedB);
        ...
        function applySmartOverlaySelected() {
            ...
            if (successCount > 0) closeSmartOverlay();
        }
        ...
        function showExportOptions(btn) {
            ...
            pop.style.padding = '16px';
            pop.style.zIndex = '3000';
            pop.style.width = 'min(860px, calc(100vw - 24px))';
            pop.style.maxWidth = '860px';
            pop.style.maxHeight = 'calc(100vh - 24px)';
            pop.style.height = `${Math.min(640, Math.max(360, window.innerHeight - 24))}px`;
            ...
            const buildOptionButton = (id, fmt, label, minWidth) => `
                <button id="${id}" class="btn-small btn-secondary" onclick="setExportFormat('${fmt}', this)" style="min-width:${minWidth}; text-align:center; padding:8px 16px; border-radius:10px; background:#8C9CB2; color:#FFFFFF; font-weight:600; border:none;">${label}</button>
            `;
            const buildGroup = (groupId, title, desc, buttons) => `
                <div id="${groupId}" style="padding:12px; border-radius:12px; background:${isLight ? '#F8FAFC' : 'rgba(15, 23, 42, 0.46)'}; border:${isLight ? '1px solid #E2E8F0' : '1px solid rgba(148, 163, 184, 0.18)'};">
                    <div style="display:flex; gap:8px 12px; flex-wrap:wrap; align-items:center;">
                        <div style="font-size:14px; font-weight:600; color:${isLight ? '#0f172a' : '#FFFFFF'}; white-space:nowrap;">${title}</div>
                        <div style="font-size:12px; color:${isLight ? '#64748B' : '#94A3B8'}; flex:1 1 260px; min-width:180px;">${desc}</div>
                    </div>
            ...
            pop.innerHTML = `
                ...
                <div id="exportOptions" style="display:grid; gap:12px;">
                    ${buildGroup(
                        'group-original-view',
                        '原始关系导出',
                        '保留现有拍平关系格式，适合逐条关系核对和继续处理。',
                        [
                            buildOptionButton('opt-json', 'json', 'JSON', '88px'),
                            buildOptionButton('opt-csv', 'csv', 'CSV', '88px'),
                            buildOptionButton('opt-xlsx', 'xlsx', 'Excel', '88px')
                        ].join('')
                    )}
                    ${buildGroup(
                        'group-source-view',
                        'A 主视角导出',
                        '每个 A 节点仅展示一行，多个 B 节点信息使用英文逗号分隔。',
                        [
                            buildOptionButton('opt-source-csv', 'source_csv', 'CSV', '100px'),
                            buildOptionButton('opt-source-xlsx', 'source_xlsx', 'Excel', '100px')
                        ].join('')
                    )}
                    ${buildGroup(
                        'group-target-view',
                        'B 主视角导出',
                        '每个 B 节点仅展示一行，多个 A 节点信息使用英文逗号分隔。',
                        [
                            buildOptionButton('opt-target-csv', 'target_csv', 'CSV', '100px'),
                            buildOptionButton('opt-target-xlsx', 'target_xlsx', 'Excel', '100px')
                        ].join('')
                    )}
                </div>
                <div id="export-preview-panel" style="padding:12px; border-radius:12px; background:${isLight ? '#F8FAFC' : 'rgba(15, 23, 42, 0.46)'}; border:${isLight ? '1px solid #E2E8F0' : '1px solid rgba(148, 163, 184, 0.18)'}; margin-top:12px; margin-bottom:16px; min-height:0; flex:1 1 auto; overflow:hidden; display:flex; flex-direction:column;">
                    <div style="font-size:14px; font-weight:600; color:${isLight ? '#0f172a' : '#FFFFFF'};">示例数据预览</div>
                    <div id="export-preview-hint" style="margin-top:6px; font-size:12px; color:${isLight ? '#64748B' : '#94A3B8'};"></div>
                    <div id="export-preview-body" style="margin-top:10px; min-height:0; flex:1 1 auto; overflow:hidden;"></div>
                </div>
                <div style="display:flex; gap:8px; justify-content:flex-end; flex-shrink:0; margin-top:auto;">
            ...
            const vw = window.innerWidth; const vh = window.innerHeight; const pad = 6;
            const width = Math.max(pop.offsetWidth, 220);
            const height = Math.min(Math.max(pop.offsetHeight, 60), vh - pad * 2);
            let left = rect.left;
            let top = rect.bottom + 8;
            if (left + width > vw - pad) left = Math.max(pad, vw - width - pad);
            if (top + height > vh - pad) top = Math.max(pad, rect.top - height - pad);
            if (top + height > vh - pad) top = pad;
            pop.style.left = left + 'px';
            pop.style.top = top + 'px';
            const remove = (e) => { if (!pop.contains(e.target)) { pop.remove(); document.removeEventListener('click', remove, true); } };
            setTimeout(() => document.addEventListener('click', remove, true), 0);
        ...
        function setExportFormat(fmt) {
            state.exportFormat = ['json', 'csv', 'xlsx', 'source_csv', 'source_xlsx', 'target_csv', 'target_xlsx'].includes(fmt) ? fmt : 'json';
            updateExportOptionsUI();
        }
        ...
        function updateExportOptionsUI() {
            ...
            const sourceCsvBtn = document.getElementById('opt-source-csv');
            const sourceXlsxBtn = document.getElementById('opt-source-xlsx');
            const targetCsvBtn = document.getElementById('opt-target-csv');
            const targetXlsxBtn = document.getElementById('opt-target-xlsx');
            const confirmBtn = document.getElementById('export-confirm');
            const cancelBtn = document.getElementById('export-cancel');
            if (!jsonBtn || !csvBtn || !xlsxBtn || !sourceCsvBtn || !sourceXlsxBtn || !targetCsvBtn || !targetXlsxBtn || !confirmBtn || !cancelBtn) return;
            ...
            const buttons = [
                ...
                { el: sourceCsvBtn, active: activeFormat === 'source_csv' },
                { el: sourceXlsxBtn, active: activeFormat === 'source_xlsx' },
                { el: targetCsvBtn, active: activeFormat === 'target_csv' },
                { el: targetXlsxBtn, active: activeFormat === 'target_xlsx' }
            ];
            ...
            buttons.forEach(({ el, active }) => {
                ...
                el.style.minWidth = (el.id.includes('source') || el.id.includes('target')) ? '100px' : '88px';
        ...
        function renderExportPreview() {
            const hintEl = document.getElementById('export-preview-hint');
            const bodyEl = document.getElementById('export-preview-body');
            if (!hintEl || !bodyEl) return;
            const fmt = state.exportFormat || 'json';
            const meta = buildExportPreviewMeta(fmt);
            hintEl.innerHTML = `<strong>${escapeHtml(meta.title)}</strong> · ${escapeHtml(meta.summary)}`;
            if (fmt === 'json') {
                bodyEl.innerHTML = buildJsonPreviewHtml(buildExportJsonPreviewPayload());
            } else {
                const rows = buildExportPreviewRows(fmt);
                bodyEl.innerHTML = buildStructuredPreviewHtml(meta.columns, rows, meta.descriptions);
            }
        }

        function buildExportPreviewMeta(format) {
            const rawTitle = format === 'xlsx' ? '原始关系导出 / Excel' : '原始关系导出 / CSV';
            const sourceTitle = format === 'source_xlsx' ? 'A 主视角导出 / Excel' : 'A 主视角导出 / CSV';
            const targetTitle = format === 'target_xlsx' ? 'B 主视角导出 / Excel' : 'B 主视角导出 / CSV';
            const previewMap = {
                json: {
                    title: '原始关系导出 / JSON',
                    summary: '导出完整映射对象和元数据，适合备份与二次导入。',
                    columns: [],
                    descriptions: []
                },
                csv: {
                    title: rawTitle,
                    summary: '每一行代表一条 A-B 关系，适合逐条核对映射。', 
                    columns: [
                        { key: 'sourceId', label: 'sourceId' },
                        { key: 'sourceName', label: 'sourceName' },
                        { key: 'sourceLevel', label: 'sourceLevel' },
                        { key: 'targetId', label: 'targetId' },
                        { key: 'targetName', label: 'targetName' },
                        { key: 'targetLevel', label: 'targetLevel' }
                    ],
                    descriptions: [
                        { label: 'sourceId', text: '平台 A 节点 ID。' },
                        { label: 'sourceName', text: '平台 A 节点名称。' },
                        { label: 'sourceLevel', text: '平台 A 节点层级。' },
                        { label: 'targetId', text: '平台 B 节点 ID。' },
                        { label: 'targetName', text: '平台 B 节点名称。' },
                        { label: 'targetLevel', text: '平台 B 节点层级。' }
                    ]
                },
                xlsx: {
                    title: rawTitle,
                ...
                source_csv: {
                    title: sourceTitle,
                    summary: '每个 A 节点仅一行，多个 B 节点信息使用英文逗号连接。',
                    columns: [
                        { key: 'sourceId', label: 'sourceId' },
                        { key: 'sourceName', label: 'sourceName' },
                        { key: 'sourceLevel', label: 'sourceLevel' },
                        { key: 'targetIds', label: 'targetIds' },
                        { key: 'targetNames', label: 'targetNames' },
                        { key: 'targetDisplay', label: 'targetDisplay' }
                    ],
                    descriptions: [
                        { label: 'sourceId', text: '平台 A 节点 ID。' },
                        { label: 'sourceName', text: '平台 A 节点名称。' },
                        { label: 'sourceLevel', text: '平台 A 节点层级。' },
                        { label: 'targetIds', text: '该 A 节点绑定的所有 B 节点 ID，使用英文逗号分隔。' },
                        { label: 'targetNames', text: '该 A 节点绑定的所有 B 节点名称，使用英文逗号分隔。' },
                        { label: 'targetDisplay', text: '按 名称(ID) 聚合的 B 节点展示字段。' }
                    ]
                },
                source_xlsx: {
                    title: sourceTitle,
                ...
                target_csv: {
                    title: targetTitle,
                    summary: '每个 B 节点仅一行，多个 A 节点信息使用英文逗号连接。',
                    columns: [
                        { key: 'targetId', label: 'targetId' },
                        { key: 'targetName', label: 'targetName' },
                        { key: 'targetLevel', label: 'targetLevel' },
                        { key: 'sourceIds', label: 'sourceIds' },
                        { key: 'sourceNames', label: 'sourceNames' },
                        { key: 'sourceDisplay', label: 'sourceDisplay' }
                    ],
                    descriptions: [
                        { label: 'targetId', text: '平台 B 节点 ID。' },
                        { label: 'targetName', text: '平台 B 节点名称。' },
                        { label: 'targetLevel', text: '平台 B 节点层级。' },
                        { label: 'sourceIds', text: '该 B 节点绑定的所有 A 节点 ID，使用英文逗号分隔。' },
                        { label: 'sourceNames', text: '该 B 节点绑定的所有 A 节点名称，使用英文逗号分隔。' },
                        { label: 'sourceDisplay', text: '按 名称(ID) 聚合的 A 节点展示字段。' }
                    ]
                },
                target_xlsx: {
                    title: targetTitle,
        ...
        function buildExportPreviewRows(format) {
            ...
            return rows.length > 0 ? rows.slice(0, 3) : getFallbackPreviewRows(format);
        }

        function buildExportJsonPreviewPayload() {
            const mappings = Array.isArray(state.mappings) ? state.mappings : [];
            const previewMappings = mappings.length > 0
                ? mappings.slice(0, 2).map(mapping => ({
                    sourceId: mapping.sourceId,
                    sourceName: mapping.sourceName,
                    targetIds: Array.isArray(mapping.targetIds) ? mapping.targetIds : [],
                    targetNames: Array.isArray(mapping.targetNames) ? mapping.targetNames : []
                }))
                : [
                    {
                        sourceId: 'A-1',
            ...
            return {
                ...
                platformNameA: state.ui.platformNameA || '平台A',
                platformNameB: state.ui.platformNameB || '平台B',
                mappings: previewMappings
            };
        }

        function getExportRowsForFormat(format) {
            if (format === 'csv' || format === 'xlsx') return buildMappingExportRows();
            if (format === 'source_csv' || format === 'source_xlsx') return buildSourceCentricExportRows();
            if (format === 'target_csv' || format === 'target_xlsx') return buildTargetCentricExportRows();
            return [];
        }

        function getFallbackPreviewRows(format) {
            const fallbackMap = {
                csv: [
                    { sourceId: 'A-1', sourceName: '代数', sourceLevel: 2, targetId: 'B-1', targetName: '方程', targetLevel: 3 },
                    { sourceId: 'A-1', sourceName: '代数', sourceLevel: 2, targetId: 'B-2', targetName: '函数', targetLevel: 3 },
                    { sourceId: 'A-2', sourceName: '几何', sourceLevel: 2, targetId: 'B-3', targetName: '平面几何', targetLevel: 3 }
                ],
                xlsx: [
                    { sourceId: 'A-1', sourceName: '代数', sourceLevel: 2, targetId: 'B-1', targetName: '方程', targetLevel: 3 },
                    { sourceId: 'A-1', sourceName: '代数', sourceLevel: 2, targetId: 'B-2', targetName: '函数', targetLevel: 3 },
                ...
                source_csv: [
                    { sourceId: 'A-1', sourceName: '代数', sourceLevel: 2, targetIds: 'B-1,B-2', targetNames: '方程,函数', targetDisplay: '方程(B-1),函数(B-2)' },
                    { sourceId: 'A-2', sourceName: '几何', sourceLevel: 2, targetIds: 'B-3', targetNames: '平面几何', targetDisplay: '平面几何(B-3)' }
                ],
                source_xlsx: [
                    { sourceId: 'A-1', sourceName: '代数', sourceLevel: 2, targetIds: 'B-1,B-2', targetNames: '方程,函数', targetDisplay: '方程(B-1),函数(B-2)' },
                    { sourceId: 'A-2', sourceName: '几何', sourceLevel: 2, targetIds: 'B-3', targetNames: '平面几何', targetDisplay: '平面几何(B-3)' }
                ...
                target_csv: [
                    { targetId: 'B-1', targetName: '方程', targetLevel: 3, sourceIds: 'A-1,A-2', sourceNames: '代数,几何', sourceDisplay: '代数(A-1),几何(A-2)' },
                    { targetId: 'B-2', targetName: '函数', targetLevel: 3, sourceIds: 'A-1', sourceNames: '代数', sourceDisplay: '代数(A-1)' }
                ],
                target_xlsx: [
                    { targetId: 'B-1', targetName: '方程', targetLevel: 3, sourceIds: 'A-1,A-2', sourceNames: '代数,几何', sourceDisplay: '代数(A-1),几何(A-2)' },
                    { targetId: 'B-2', targetName: '函数', targetLevel: 3, sourceIds: 'A-1', sourceNames: '代数', sourceDisplay: '代数(A-1)' }
            ...
            return fallbackMap[format] || fallbackMap.json;
        }

        function buildJsonPreviewHtml(payload) {
            return `
                <div id="export-preview-scroll" style="height:100%; overflow:auto;">
                    <pre style="margin:0; padding:12px; border-radius:10px; background:${(state.ui && state.ui.theme) === 'light' ? '#E2E8F0' : '#111827'}; color:${(state.ui && state.ui.theme) === 'light' ? '#0F172A' : '#E2E8F0'}; font-size:12px; line-height:1.55; white-space:pre-wrap; word-break:break-word; overflow:auto;"><code>${escapeHtml(JSON.stringify(payload, null, 2))}</code></pre>
                </div>
            `;
        }
        ...
        function buildStructuredPreviewHtml(columns, rows, descriptions) {
            return `
                <div id="export-preview-scroll" style="height:100%; overflow:auto; padding-right:2px;">
                    <section id="preview-table-section">
                        <div style="font-size:12px; font-weight:600; color:${(state.ui && state.ui.theme) === 'light' ? '#334155' : '#E2E8F0'}; margin-bottom:8px;">示例数据</div>
                        ${buildPreviewTableHtml(columns, rows)}
                    </section>
                    <div id="preview-divider" style="height:1px; background:${(state.ui && state.ui.theme) === 'light' ? '#CBD5E1' : 'rgba(148, 163, 184, 0.32)'}; margin:14px 0;"></div>
                    <section id="preview-desc-section">
                        <div style="font-size:12px; font-weight:600; color:${(state.ui && state.ui.theme) === 'light' ? '#334155' : '#E2E8F0'}; margin-bottom:8px;">字段说明</div>
                        ${buildPreviewDescriptionsHtml(descriptions)}
                    </section>
                </div>
            `;
        }

        function buildPreviewTableHtml(columns, rows) {
            const headHtml = columns.map(column => `<th style="padding:8px 10px; text-align:left; white-space:nowrap; border-bottom:1px solid rgba(148, 163, 184, 0.25);">${escapeHtml(column.label)}</th>`).join('');
            const bodyHtml = rows.map(row => `
                <tr>
                    ${columns.map(column => `<td style="padding:8px 10px; vertical-align:top; border-bottom:1px solid rgba(148, 163, 184, 0.16);">${escapeHtml(row[column.key])}</td>`).join('')}
                </tr>
            `).join('');
            return `
        ...
        function buildPreviewDescriptionsHtml(descriptions) {
            return descriptions.map(item => `
                <div style="font-size:12px; color:${(state.ui && state.ui.theme) === 'light' ? '#475569' : '#CBD5E1'}; padding:8px 0; border-top:1px solid ${(state.ui && state.ui.theme) === 'light' ? 'rgba(203, 213, 225, 0.7)' : 'rgba(148, 163, 184, 0.18)'};">
        ...
        function confirmExport() {
            ...
            try {
                ...
                else if (fmt === 'source_csv') exportSourceCentricCSV();
                else if (fmt === 'source_xlsx') exportSourceCentricExcel();
                else if (fmt === 'target_csv') exportTargetCentricCSV();
        ...
        function buildMappingExportRows() {
            const rows = [];
            state.mappings.forEach(m => {
                const sourceLevel = getNodeLevelById(state.platformA, m && m.sourceId);
                (m.targetIds || []).forEach((tid, idx) => {
                    ...
                    rows.push({
                        ...
                        targetId: tid,
                        targetName: tname,
                        targetLevel: getNodeLevelById(state.platformB, tid)
                    });
                });
            });
            return rows;
        ...
        function buildSourceCentricExportRows() {
            const rows = [];
            const rowMap = new Map();
            state.mappings.forEach(m => {
                if (!m || !m.sourceId) return;
                let row = rowMap.get(m.sourceId);
                if (!row) {
                    row = {
                        sourceId: m.sourceId,
                        sourceName: m.sourceName || '',
                        sourceLevel: getNodeLevelById(state.platformA, m.sourceId),
                        targetIds: [],
                        targetNames: [],
                        targetDisplay: []
                    };
                    rowMap.set(m.sourceId, row);
                    rows.push(row);
                }
                if (!row.sourceName && m.sourceName) row.sourceName = m.sourceName;
                (m.targetIds || []).forEach((tid, idx) => {
                    if (tid == null || tid === '') return;
                    const targetId = String(tid);
                    if (row.targetIds.includes(targetId)) return;
                    const targetName = String((m.targetNames || [])[idx] || '');
                    row.targetIds.push(targetId);
                    row.targetNames.push(targetName);
                    row.targetDisplay.push(targetName ? `${targetName}(${targetId})` : `(${targetId})`);
                });
            });
            return rows.map(row => ({
                ...
                targetNames: row.targetNames.join(','),
                targetDisplay: row.targetDisplay.join(',')
            }));
        }
        ...
        function buildTargetCentricExportRows() {
            ...
            state.mappings.forEach(m => {
                ...
                (m.targetIds || []).forEach((tid, idx) => {
                    ...
                    const targetId = String(tid);
                    const targetName = String((m.targetNames || [])[idx] || '');
                    let row = rowMap.get(targetId);
                    if (!row) {
                        row = {
                            targetId,
                            targetName,
                            targetLevel: getNodeLevelById(state.platformB, targetId),
                            sourceIds: [],
                            sourceNames: [],
                            sourceDisplay: []
                        };
                    ...
                    if (!row.targetName && targetName) row.targetName = targetName;
                    const sourceId = String(m.sourceId || '');
                    if (!sourceId || row.sourceIds.includes(sourceId)) return;
                    const sourceName = String(m.sourceName || '');
                    row.sourceIds.push(sourceId);
                    row.sourceNames.push(sourceName);
                    row.sourceDisplay.push(sourceName ? `${sourceName}(${sourceId})` : `(${sourceId})`);
                });
            });
            return rows.map(row => ({
        ...
        function exportMappingsCSV() {
            ...
            downloadCSVRows(['sourceId', 'sourceName', 'sourceLevel', 'targetId', 'targetName', 'targetLevel'], exportRows, `${state.ui.platformNameA}_${state.ui.platformNameB}关系映射 ${formatDateYMD(new Date())}.csv`);
        }
        ...
        function exportMappingsExcel() {
            ...
            exportRowsToExcel(['sourceId', 'sourceName', 'sourceLevel', 'targetId', 'targetName', 'targetLevel'], rows, 'Mappings', `${state.ui.platformNameA}_${state.ui.platformNameB}关系映射 ${formatDateYMD(new Date())}.xlsx`);
        }

        function exportSourceCentricCSV() {
            if (state.mappings.length === 0) { alert('没有映射关系可导出！'); return; }
            const exportRows = buildSourceCentricExportRows();
            downloadCSVRows(['sourceId', 'sourceName', 'sourceLevel', 'targetIds', 'targetNames', 'targetDisplay'], exportRows, `${state.ui.platformNameA}_${state.ui.platformNameB}A主视角关系映射 ${formatDateYMD(new Date())}.csv`);
        }

        function exportSourceCentricExcel() {
            if (state.mappings.length === 0) { alert('没有映射关系可导出！'); return; }
            if (typeof XLSX === 'undefined') {
                throw new Error('Excel导出库加载失败，请刷新页面重试');
            }
            const rows = buildSourceCentricExportRows();
            exportRowsToExcel(['sourceId', 'sourceName', 'sourceLevel', 'targetIds', 'targetNames', 'targetDisplay'], rows, 'SourceMappings', `${state.ui.platformNameA}_${state.ui.platformNameB}A主视角关系映射 ${formatDateYMD(new Date())}.xlsx`);
        }

        function exportTargetCentricCSV() {
            if (state.mappings.length === 0) { alert('没有映射关系可导出！'); return; }
            const exportRows = buildTargetCentricExportRows();
            downloadCSVRows(['targetId', 'targetName', 'targetLevel', 'sourceIds', 'sourceNames', 'sourceDisplay'], exportRows, `${state.ui.platformNameA}_${state.ui.platformNameB}B主视角关系映射 ${formatDateYMD(new Date())}.csv`);
        }
        ...
        function exportTargetCentricExcel() {
            ...
            const rows = buildTargetCentricExportRows();
            exportRowsToExcel(['targetId', 'targetName', 'targetLevel', 'sourceIds', 'sourceNames', 'sourceDisplay'], rows, 'TargetMappings', `${state.ui.platformNameA}_${state.ui.platformNameB}B主视角关系映射 ${formatDateYMD(new Date())}.xlsx`);
        }
        ...
        function exportRowsToExcel(headers, rows, sheetName, fileName) {
        ...
        function applySmartMappings() {
            ...
            const appliedCount = results.filter(result => result && result.success).length;
            
            if (appliedCount > 0) {
                addToHistory('智能批量映射', `应用了${appliedCount}个映射`, '');
            }
            showToast(buildBindSummaryToast(results), appliedCount > 0 ? 'success' : 'error');
